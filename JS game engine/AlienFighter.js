class AlienFighter extends NPC {
  ...
  constructor(playerPosition, canvasWidth, canvasHeight) {
    ...
    const margin = AlienFighter.size.x * AlienFighter.SPAWN_MARGIN_MULTIPLIER;
  ...
  pickNewTarget(playerPosition) {
    ...
    const distance = Math.random() * screenRadius * AlienFighter.TARGET_AREA_FACTOR;
  ...
  hasReachedTarget() {
    ...
    return distance < AlienFighter.ARRIVAL_THRESHOLD;
  }
  ...
  turnTowards(targetAngle, deltaTime) {
    ...
    if (Math.abs(angleDiff) > AlienFighter.TURN_PRECISION) {
      if (angleDiff > 0) {
  ...
  update(deltaTime, playerPosition) {
    ...
    if (this.isOffScreen(playerPosition)) {
      ...
      if (this.offScreenTime > AlienFighter.OFFSCREEN_TIMEOUT) {
        this.pickNewTarget(playerPosition);
        this.offScreenTime = 0;
      }
    } else {
    ...
    if (this.targetPosition) {
      ...
      const targetAngle = Math.atan2(toTarget.y, toTarget.x) + AlienFighter.SPRITE_UP_ANGLE_OFFSET;
